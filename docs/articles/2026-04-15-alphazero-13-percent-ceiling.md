...
我跑了几周。loss 曲线漂亮得像教科书：平滑下降，没有震荡，没有发散。看起来一切正常，我满怀信心地让它和随机对手打了 50 局——连随机都赢不了的话，那才叫有问题吧？
...
**概念搞混了。** 训练循环里有个参数叫 `num_epochs`，我设成了 100，觉得"数据过 100 遍，够多了"。但翻开 for 循环一看，它并不是"遍历整个数据集"的 epoch——每一步只随机抽一个 batch（128 个样本）做一次梯度更新，跑 100 步就结束。`num_epochs=100` 听起来是 100 个 epoch，做的事情是 100 个 gradient step。

问题出在我混淆了三个基本概念：取一小批样本做一次梯度更新叫一个 **gradient step**，把整个训练集过一遍叫一个 **epoch**，每次喂进网络的那一小批样本叫一个 **batch**。三者的关系是：一个 epoch = 数据集大小 / batch_size 个 gradient step。假设训练 buffer 里有 5000 条样本、batch_size 是 128，那一个真正的 epoch 需要约 39 个 gradient step。我写的"100 个 epoch"？只是 100 个 gradient step——连 3 个真正的 epoch 都不到。
...
**加网络容量。** 第二个念头：也许网络太小了。我一度以为模型越深越好——更多的残差块意味着更强的特征提取能力。但算了一下卷积的感受野才发现问题。alpha-zero-general 用的是四层残差块，感受野 9x9——对 8x8 的标准棋盘刚好够覆盖全局。而我的 6x6 棋盘比 8x8 还小，9x9 的感受野早就超出棋盘边界了。网络已经能"看到"整个棋盘的每一个角落，再加深只是在做冗余计算。问题不在深度，那也许在宽度？128 个 channel 也许不足以编码足够细的棋形特征。我把 channels 扩到 256，参数量翻了差不多四倍，顺手加了 dropout 防止过拟合。训练跑起来之后，loss 确实更低了——策略头和价值头的损失都比之前降了一截，曲线干净利落。更大的网络在训练集上拟合得更好。我带着一点期待去跑 vs random 评估。
...
50 局全赢。不是 60%，不是 80%——100%。从第一个迭代就开始碾压随机对手，到第 8 个迭代已经找不到对手了。alpha-zero-general 的默认配置是跑 1000 个迭代，但在我的 6x6 棋盘上，只训了 30 个迭代，我自己就再也赢不了它了。之前调了三周的所有超参、加的网络容量、修的代码 bug，都没这一行管用。
...
**2. 搞清楚基本概念，然后逐行对齐参考实现。** 我把 gradient step 当成了 epoch，因为变量名叫 `num_epochs`。这不是变量名的问题，是我自己没分清 epoch、batch、gradient step 这三个概念的边界。概念模糊的时候，代码怎么写都觉得"看起来没问题"。照着参考实现逐行对齐，是最没面子但最有效的 debug 动作——不是对比"高层结构差不多"，而是对比每一层 for 循环到底在遍历什么。

**3. 当调参和 review 都无效，怀疑你的数据表示。** 算法教科书讲的是"它应该怎么算"，数据结构决定了"它实际在算什么"。两个 MCTS 实现可以算法完全等价、渐近复杂度相同，但一个训出 100% 胜率，另一个永远卡在 13%。区别只是 dict 还是 tree——一个能识别同构局面，一个不能。
