...
围棋太贵了——AlphaGo Zero 用了 5000 块 TPU 训了 40 天。我选了 Othello（黑白棋），一个规则简单但策略深度足够的棋类。为了进一步压缩计算量，棋盘从标准的 8x8 缩小到 6x6。社区里有一个成熟的参考实现 alpha-zero-general，支持多种棋类，我打算以它为蓝本，在自己的笔记本上跑完整个训练流程。
