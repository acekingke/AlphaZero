...
社区里有一个成熟的参考实现 alpha-zero-general，支持多种棋类。我以它为蓝本，在自己的笔记本上搭好了训练框架。AlphaZero 的训练循环并不复杂：自我对弈产生棋谱数据，神经网络从中学习策略和局面评估，然后让新模型和旧模型在 Arena 里打一组对局——胜率超过 60% 才接受新模型，否则回滚。循环往复，理论上模型会越来越强。

我跑了几周。loss 曲线漂亮得像教科书：平滑下降，没有震荡，没有发散。我觉得差不多了，让它和一个"每步随机选一个合法位置落子"的程序打了 50 局。
