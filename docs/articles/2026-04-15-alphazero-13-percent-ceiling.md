...
**加网络容量。** 第二个念头：也许网络太小了。我一度以为模型越深越好——更多的残差块意味着更强的特征提取能力。但算了一下卷积的感受野才发现，四层 3x3 的残差块叠起来，理论感受野已经是 19x19，远远超过 6x6 的棋盘。网络早就能"看到"整个棋盘了，再加深只是在做冗余计算。问题不在深度，那也许在宽度？128 个 channel 也许不足以编码足够细的棋形特征。我把 channels 扩到 256，参数量翻了差不多四倍，顺手加了 dropout 防止过拟合。训练跑起来之后，loss 确实更低了——策略头和价值头的损失都比之前降了一截，曲线干净利落。更大的网络在训练集上拟合得更好。我带着一点期待去跑 vs random 评估。
