...
## MCTS 结构性 Bug：树 vs 字典（2026-04-14 复盘）

> **背景**：v11 修好均匀陷阱后，训练似乎健康（v16 原版峰值 vs_random 13.3%），但长期卡在 10-13% 区间无法突破。多次换参数（Dirichlet、epochs、threshold）都无效。最终发现**根因在 MCTS 的数据结构**。

### 这一轮实验时间线

| 版本 | 核心变化 | vs_random 峰值 | 接受/拒绝 |
|---|---|---:|---|
| v16 | 原版（树结构，无跨步复用） | 13.3%（iter 17） | 混乱震荡 |
| v17 | +树内 root 复用（`advance_root`） | 10%（iter 2） | 1 accept / 7 reject |
| v18 | +`arena_threshold` 0.6→0.5 | 10%（iter 5） | 前期 accept 正常，iter 6+ 崩 |
| v19 | +Dirichlet α 1.0→0.3, epochs 10→5 | 13.3%（iter 2）但之后降到 0% | 越训越差 |
| **v20** | **MCTS 换字典结构（对齐 alpha-zero-general）** | **iter 8 达到 100%** | 健康接受 |

### 关键诊断（当时怎么定位到的）

1. **发现 best.pt 被 revert 卡住**  
   v17 里观察到 `best.pt` 的文件时间 8 轮都没变，说明 Arena 一直 REJECT，每次 reject 都把 `self.model` reload 回 best（iter 0 权重）。**训练的候选被丢弃**。

2. **拆开测 policy 和 value**（见 `scripts/exp_pure_policy_vs_random.py`）  
   ```
   iter 0 冷网络      裸策略:  43.3%  | MCTS 25 sim:  6.7%
   iter 7 训练候选    裸策略:  43.3%  | MCTS 25 sim:  ?
   ```
   **纯策略 43% 说明网络本身没坏。MCTS 把 43% 砍到 6.7% 才是真凶**。

3. **深搜索让坏模型更糟（反直觉）**  
   同一个模型用 400 sim + 树复用 vs random：**0%**。如果 value head 坏了，深搜索越坚定往输棋路径走。这进一步把怀疑集中到 MCTS。

4. **字典 MCTS 直接验证**  
   拿 v19 iter 2 那个 13.3% 的模型，换字典 MCTS 重测（同样 25 sim）：**63.3%**。模型一字没动，MCTS 一换就 5x 提升。铁证。

### 为什么字典结构这么关键

**树结构**（原代码）：每个节点通过 `children` 字典串起来，同一局面通过不同走法到达 = 两个独立节点，**统计不共享**。

**字典结构**（alpha-zero-general）：`Qsa/Nsa/Ns/Ps/Vs` 以"局面字符串"为 key，**任意路径到同一局面 = 共享同一组统计**。

在 6x6 Othello 里：
- 一个游戏常见局面数大约 10^6 级，但 25 次模拟能访问到的**不同局面**只有几十个
- 跨路径共享 → 每个关键局面在 25 sim 里可能被访问 3-5 次，**有效信息远高于表面访问数**
- alpha-zero-general 的 Arena 还跨 40 局不清空 dict → 常见开局位置累积数千次访问 → 等效搜索深度远 > 25

**树结构在一局内无法共享，在 Arena 跨局更无法共享。所以每次 25 sim 真的就只是 25 sim，而且同一关键局面只被探到 1-2 次 → 策略先验支配一切 → 一旦先验有偏 MCTS 毫无纠错能力。**

### 修复代价与效果

- 代码量：~230 行（重写 `mcts/mcts.py`，MCTS 从 `Node` 树结构改为 `Qsa/Nsa/Ns/Ps/Vs` 6 个字典）
- 接口保持不变：`search(state, env, temperature, add_noise) → probs`，`reset_tree()`，`advance_root(action)`（字典下变成 no-op）
- **效果**：v19 iter 2 模型的 MCTS 胜率 13.3% → 63.3%（5x）
- **训练效果**：v20 iter 8 vs_random 100%，8 轮就突破了 v16 花 17 轮才到的 13.3% 天花板

### 给未来自己的清单

重启训练前，除了均匀陷阱检查，加上**MCTS 结构性检查**：

- [ ] 我的 MCTS 是否跨路径共享统计？（看数据结构是 Node 树还是 state-keyed dict）
- [ ] 同一局面通过不同走法到达时，Qsa 是独立的还是共享的？
- [ ] Arena 的 40 局之间 dict 是否累积？（alpha-zero-general: 累积；清空就是 bug）
- [ ] 把 vs_random 评估拆成"纯策略 vs MCTS"两个数据点分别看，差 5x 以上就是 MCTS 问题
- [ ] 如果策略头在裸测下表现 > 40%，但 MCTS 下 < 20%，不是网络问题

**同样的规则**：代码 review 看不出"树 vs 字典"哪个对，看文档也不会明显告诉你。只有**把两种实现在同一个模型上跑对比**才能看到 5x 的差距。

### 为什么几个月没发现这个

1. 树实现"看起来像"MCTS（有 Node、children、visit_count、PUCT），通过了所有已有测试
2. 参考实现（alpha-zero-general）用了不同的抽象，日常读代码不会对齐到字典层面
3. 树实现在**单个模型固定时的 pure-policy** 测试正常（43%）→ 检查网络时不会怀疑 MCTS
4. vs_random **两个实现都接近 0-10%** 区间 → 数字看起来差不多，掩盖了 5x 差距

**"看起来差不多"是最危险的信号**。0.2→0.3 只是 1.5x，但对可学习的上限而言，多出的 50% 搜索能力决定是否能建立起正反馈训练循环。
...
> **补充（2026-04-14）**：即使方法论都对、参数都对齐参考实现，**数据结构层面的偏差也能让训练卡死**。检查清单里必须有"我的数据结构和参考实现在语义上等价吗？"这一项。
