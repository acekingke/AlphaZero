...
#### 信噪比：为什么 1 epoch 学不到信号

**简化模型**：每个训练样本的 target = signal + noise：

```
signal 的幅度 ≈ 0.02 (真实的 2% 偏好差异)
noise 的幅度 ≈ 0.05 (Dirichlet 噪声 + 采样波动，比 signal 还大)
```

**单次看样本**，noise > signal。梯度会朝错误方向走。
...
**10 epochs**（每样本看 10 次，每次 noise 不同）：
- 平均梯度 ≈ signal + noise / √10
- 0.02 + 0.05/3.16 ≈ 0.02 + 0.016
- **信号首次大于平均噪声**
- NN 终于能学到那 2% 偏好

这是标准的**信噪比通过重复平均提升 √N 倍**的原理。
