...
def test_network_output():
    ...
    checkpoint = torch.load("models/checkpoint_30.pt", map_location=device)
    ...
    for i, action in enumerate(top_actions):
        if action == env.size * env.size:
            print(
                f"  {i + 1}. Pass (action {action}): prob={policy[action]:.4f}, valid={valid_moves_mask[action]}"
            )
        else:
            row, col = env.get_coords_from_action(action)
            is_valid = valid_moves_mask[action]
            print(
                f"  {i + 1}. ({row},{col}) (action {action}): prob={policy[action]:.4f}, valid={is_valid}"
            )
