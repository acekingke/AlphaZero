...
def play_game(black_player, white_player, render=True):
    ...
    while not done:
        # Get current player
        current_player = (
            black_player if env.board.current_player == -1 else white_player
        )

        if render:
            print(
                f"{'Black' if env.board.current_player == -1 else 'White'} player's turn"
            )
...
def main():
    ...
    parser.add_argument(
        ...
        default="black",
        choices=["black", "white"],
        help="Player color (black or white)",
    )
    parser.add_argument(
        "--mcts_simulations", type=int, default=25, help="Number of MCTS simulations"
    )
    args = parser.parse_args()

    # Create players
    alphazero_player = AlphaZeroPlayer(
        args.model, num_simulations=args.mcts_simulations
    )
